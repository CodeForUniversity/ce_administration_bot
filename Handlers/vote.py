...
async def vote_yes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    voter_id = update.effective_user.id
    chat_id = update.effective_chat.id

    # Find open session in this chat
    session = (
    ...
    session, status = vote_service.cast_vote(session.id, voter_id)
    ...
    if status == "voted":
        count = len(session.votes)
        return await update.message.reply_text(f"Vote recorded ({count}/20).")
    if status == "completed":
        # Mute user
        ...
        return await update.message.reply_text(
            "User muted for 1 day. Vote threshold reached."
        )
